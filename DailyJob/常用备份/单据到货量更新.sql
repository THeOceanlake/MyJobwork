...
-- Step 1: 取要货单和采购
	...
	join tmp_spb c on b.sFdbh=c.sFdbh and b.sspbh=c.sSpbh
...
--Step 2: 取进出中要货和采购的到货类型（从标准进出表中排除调拨 盘点，损溢） drop table #Jcmx
	...
	from Tmp_Jcb a,Tmp_Jcmxb b,Purchase_jcflbzb c,Tmp_FDB d,tmp_spb e  where a.sJcbh=b.sJcbh and a.sJcfl=c.fl_content and c.fl_bzname not in ('盘点','调拨','损溢')
	and a.dSj>=CONVERT(date,GETDATE()-@day) and  a.dSj<=CONVERT(date,GETDATE()) and b.nSl<>0
	and a.sFdbh=d.sFDBH and d.sFDLX<>'仓库'  and b.sFdbh=e.sFdbh and b.sSpbh=e.sSpbh
...
-- Step 3:对进出范围内关联的单据排序，然后每条记录进行分配，如果配送和非配送能分开，则分开进行，如果不能就合并处理 drop table #tmp_dj
 ...
 -- Step 3 :更新实体要货表和采购表
	...
	select *,case when a.stype='配送节奏' and  SUBSTRING(isnull(a.wwwadr,'1111101'),DATEPART(WEEKDAY,b.drq)-1,1)='1' then 'Y'
	when  a.stype='星期' and
	CHARINDEX( '0'+convert(varchar(10),case when DATEPART(WEEKDAY,b.drq)-1=0 then 7 else DATEPART(WEEKDAY,b.drq)-1 end),
	  a.wwwadr)>0 then 'Y'
    when  a.stype='天' and
	 CHARINDEX( REPLICATE( '0',2-len( DATEPART(DAY,b.drq)))    +convert(varchar(10) , DATEPART(DAY,b.drq)),
	  a.wwwadr)>0 then 'Y'
	  end  from #jz_raw a ,#tmp_date b
	where 1=1;