...
/*
  ...
  适用范围  门店一次性大范围调整及 部分商品推荐的落实
    新品进入：状态开通-无库存，已经引进-状态开通且有库存，未执行-状态未开
    淘汰单品：未执行-状态正常，执行-状态商品状态关闭
    排除  2101工业面包、          散装联营： 2306、2307、 25  烟 2201

*/
-- Step 1: 取建议数据
select  分店编号 sFdbh,分店名称 sFdmc,商品编号 sSpbh,商品名称 sSpmc,建议 sJyl
 into #1  from dbo.Tmp_Md_Sptz a 
 join dbo.goods b on a.商品编号=b.code  where 建议<>'停购淘汰'
 and  left(b.sort,4) not in ('2101','2306','2307','2201') and  left(b.sort)<>'25'
 and b.code not in  (22060171,22060174,23012425,07021299,32011103,24041721,32020231,25030491,22020139,
 23021839,24042021,25041288,25022494,25041284,23050235,58010218,31060346,24041716,24041714,25041419,
 24042201,24060132,30010355,25041418,23060205,23060003,24060028,57020538,06010363,23060206,24060036,
 75060219,06010102,06010103,06010407,24060034,23060029,06010300,06010329,24042018,30010354,46010094,
 24042017);
...
case   
	when a.sJyl='建议引进'  then case  when  a.CHARACTER_TYPE='Y' then '转联营'
      when (a.STOP_INTO='N' or a.ITEM_ETAGERE_CODE IS not null) and a.stop_sale='N' and a.CHARACTER_TYPE='N' and a.VALIDATE_FLAG='Y' 
		      then '已执行'
      when (a.STOP_INTO<>'N' and  a.ITEM_ETAGERE_CODE IS   null) or a.stop_sale<>'N' or a.CHARACTER_TYPE<>'N' or a.VALIDATE_FLAG<>'Y' 
 ...
 select a.* ,
 case when  a.ssfzx='已执行' then case when  a.sJyl='建议引进' and (isnull(a.nkcsl,0)>0 OR a.ITEM_ETAGERE_CODE is  not null)  then '已进门店'
      when a.sJyl='建议引进' and a.STOP_INTO='N' and a.ITEM_ETAGERE_CODE is   null  and isnull(a.nkcsl,0)<=0 then '开通未配货'
