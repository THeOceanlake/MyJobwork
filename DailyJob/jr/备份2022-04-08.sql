USE [DappSource_Dw]
GO
/****** Object:  StoredProcedure [dbo].[Cal_zero_bigstock]    Script Date: 2022/4/8 13:14:38 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER proc [dbo].[Cal_zero_bigstock]
as
begin
-- 自动补货商品明细
...
,e.nZdcl_Offset_Bdate,e.nZdcl_Offset_Edate,e.ifPromotion,d.qpc ncgbzs,a.min_purchase_qty nPsbzs,a.c_introduce_date,a.ITEM_ETAGERE_CODE
into #Base_sp
...
where 1=1 and   a.DEPT_CODE in ('018329','012006','012007','018389','018418','018336','018425') and a.STOP_INTO='N'
 and a.STOP_SALE='N' and a.VALIDATE_FLAG='Y' and a.CHARACTER_TYPE='N'
and b.enddate>GETDATE()-13 and (( d.sort>'20' and d.sort<'40') or (
...
into #jcmx0   from #2 a   where a.nsl<0;



select * into #3 from  DappSource_Dw.dbo.P_SALE_TOTAL_LIST_OUTPUT 
 where 1=1 and SHOP_CODE in ('018329','018336','012007','018389','018418','012006','018425') 
and CONVERT(date,SALE_DATE)>CONVERT(date,getdate()-14);


--  drop  table #xsrq

select a.sfdbh,a.sspbh,CONVERT(date,b.SALE_DATE) max_xs,SUM(b.SALE_QTY) nxssl,
ROW_NUMBER()over(partition by a.sfdbh,a.sspbh order by CONVERT(date,b.SALE_DATE) desc) npm 
into #xsrq
from #mdlkcsp a,#3 b where a.sfdbh=b.SHOP_CODE and a.sspbh=b.ITEM_CODE
group by a.sfdbh,a.sspbh,CONVERT(date,b.SALE_DATE);
...
update a set  a.syy= case when  convert(date,a.c_introduce_date)>GETDATE()-30 then  '系统定额不足' 
...
then 300 else b.CURR_STOCK*a.njj*1.0/(a.nRjxl*a.nsj) end) nzzts
from #Base_sp a 
left join DappSource_Dw.dbo.V_D_PRICE_AND_STOCK b on a.sfdbh=b.STORE_CODE and a.sspbh=b.ITEM_CODE 
where 1=1 and left(a.splbh,4) not in ('2021','2103','2104','2105','2106','2201','2203','2204','2309','3903')
 and ISNULL(b.CURR_STOCK,0)>6 and ISNULL(b.CURR_STOCK,0)*a.nJj>50 
and (( a.spsfs='配送'  and case when a.nRjxl*a.nsj=0 then 300 else 
   b.CURR_STOCK*a.njj*1.0/(a.nRjxl*a.nsj) end >55)
or ( a.spsfs<>'配送'  and case when a.nRjxl*a.nsj=0 then 300 else 
  b.CURR_STOCK*a.njj*1.0/(a.nRjxl*a.nsj) end >65)) )
...
a.nZdcl_Offset_Edate,a.nZdcl_Offset_Bdate ,CONVERT(varchar(200),'') sSource,a.ITEM_ETAGERE_CODE into #mddkc  from x0  a 
...
left join   #jcmx1 c on a.sFdbh=c.sFdbh and a.sSpbh=c.sSpbh and c.npm=2 and c.sjcfl not in('入库','调入(配送折让)','调入(配送)','调入(配送)赠品') 
where  1=1 and len(a.syy)=0;
...
nCgbzs,dzhdhr,dzhyhr,syy,sfwqzs,  case when a.spsfs='配送' then 55 else 65 end nbzzzts,a.sfcx,nDhsl,a.sSource from #mddkc a
where 1=1  ;
 ...
 -- 修改日期 20220411 任务启用 品类周转标准
 USE [DappSource_Dw]
GO
/****** Object:  StoredProcedure [dbo].[Cal_zero_bigstock]    Script Date: 2022/4/11 8:48:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
-- 商品范围
select a.SHOP_CODE sfdbh,a.ITEM_CODE sspbh,sum(a.SALE_QTY) nxssl,SUM(a.SALE_AMOUNT) nxsje into #Tmp_xs from DappSource_Dw.dbo.P_SALE_TOTAL_LIST_OUTPUT a 
where    CONVERT(date,a.SALE_DATE)>=convert(date,getdate()-30)    and  CONVERT(date,a.SALE_DATE)<CONVERT(date,GETDATE())
and a.SHOP_CODE in  ('018329','012006','012007','018389','018418','018336','018425')
...
case when a.SEND_CLASS_T='01' then '配送' when a.SEND_CLASS_T='02' then '直送'
when a.SEND_CLASS_T='03' then '一步越库' when a.SEND_CLASS_T='04' then '二步越库' end spsfs
,isnull(f.nxsje,0)/30.0 nrjxse,e.nsx nzgsl,e.nxx nzdsl,isnull(f.nxssl,0)/30.0 nrjxl,e.nzdcl,e.nzdcl_offset
,e.nZdcl_Offset_Bdate,e.nZdcl_Offset_Edate,e.ifPromotion,d.qpc ncgbzs,a.min_purchase_qty nPsbzs,a.c_introduce_date,a.ITEM_ETAGERE_CODE
into #Base_sp
from DappSource_Dw.dbo.P_SHOP_ITEM_OUTPUT a
join DappSource_Dw.dbo.goods d on a.ITEM_CODE=d.code
...
left join [122.147.10.200].dappsource.dbo.sys_deliveryset e on a.DEPT_CODE=e.sfdbh and a.ITEM_CODE=e.sspbh
left join #Tmp_xs f on a.DEPT_CODE=f.sfdbh and a.ITEM_CODE=f.sspbh
where 1=1 and   a.DEPT_CODE in ('018329','012006','012007','018389','018418','018336','018425') and a.STOP_INTO='N'
...
select a.*,ISNULL(b.nkcsl,0) nsl,CONVERT(varchar(20),'') syy,
CONVERT(datetime,'') dzhyhrq into #mdlkcsp from #Base_sp a
left join #Tmp_mdkc_sl b on  a.sfdbh=b.sfdbh and a.sspbh=b.sspbh
where ISNULL(b.nkcsl,0)<=a.nrjxl*3;
--  门店在要货数据
...
-- drop table  #mddkc
with x0 as (
select a.*,b.nkcsl nsl, convert(numeric(12,2),case when ISNULL(a.nrjxl,0)=0 
then 300 else   isnull(b.nKcsl,0)*1.0/a.nrjxl end) nzzts, isnull(c.nPlzzts,55) nPlzzts 
from #base_sp a 
left join #Tmp_mdkc_sl b on a.sfdbh=b.sfdbh and a.sspbh=b.sspbh 
left join #tmp_flbb c on LEFT(a.splbh,4)=c.sflbh 
where 1=1 and ISNULL(b.nkcsl,0)>6 and ISNULL(b.nKcje,0)>50 
and  convert(numeric(10,2),case when ISNULL(a.nrjxl,0)=0 
then 300 else isnull(b.nKcsl,0)*1.0/a.nrjxl end)>=isnull(c.nPlzzts,55) )
select a.sFdbh,a.sSpbh,a.sspmc ,a.sPlbh sFl,b.munit sdw,b.billto sGys,a.nJj,a.nSj
,a.szdbh,a.sPsfs,a.nRjxl,a.nrjxse,a.nsl,a.nzgsl,a.nzdcl
, a.nzzts,a.nPsbzs,a.nCgbzs,convert(date,Null) dzhyhr,convert(date,Null) dzhdhr, 
convert(varchar(20),'') syy,convert(varchar(5),'') sfwqzs,convert(varchar(20),'') sfcx,
convert(money,Null) nYhsl,convert(money,Null) nDhsl,a.nzdcl_offset,
...
update a set a.syy='遗留库存'   from #mddkc  a 
left join x0 b on a.sFdbh=b.sfdbh and a.sSpbh=b.sSpbh and b.npm=1 
where 1=1  and b.sFdbh is null and len(a.syy)=0;
