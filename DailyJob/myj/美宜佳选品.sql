...
--Step 3: 门店经营指标和背景指标
	...
	c.sSpbhBz,0 sState,c.nBzt,convert(money,'')   nGg,GETDATE() dCreateDate,c.spp,'' sGn,c.sGg , 0 ndc 
    into #jysp 
	from  dbo.R_Dpzb a  
	 ...
	 where 1=1 and  a.splbh in ( select sFlbh from #sflb) and  a.sFdbh in (select sFdbh from #sfd)
      ...
      select distinct 'myj'  sQybh,a.sFdbh,a.sSpbh,a.splbh sflbh,a.nJhj,a.nLsj,a.nPxs,a.sSpmc,a.sDw,a.nXssl,a.nXsje,a.nMl,c.sGys,
     c.sSpbhBz,0 nSsts,c.nBzt,convert(money,'')  nGg,GETDATE() dCreateDate,c.spp,'' sGn,c.sGg , 0 ndc
...
-- Step 4:写入数据
    -- 删除原有数据-按单品颗粒度
    delete from dbo.Input_Xp_Sp_Fd where sSpbh in (select distinct sSpbh from #jysp)
	delete from dbo.Input_Xp_Sp_All_Fd where sSpbh in (select distinct sspbh from #bjsp)
    -- 写入数据
 ...
 -- Step 5： 总指标更新
    ...
    select a.sSpbh,SUM(a.nXsje*b.pxs/(b.xse+0.00001)*0.3 + a.nml*b.pxs/(b.ml+0.00001)*0.4 + a.npxs*0.3) 
